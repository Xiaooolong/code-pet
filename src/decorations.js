...
export const decorations = {
  ...
  thinking(t) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (phase < 3.5) {
        ...
        const y = 50 - phase * 7;
        const size = 13 - i * 1.5;
        marks.push(`<text x="${x}" y="${y}" font-family="serif" font-size="${size}" fill="${getColor()}" opacity="${opacity * 0.6}">${chars[i % chars.length]}</text>`);
      }
    }
    return marks.join('');
  },
  ...
  coding(t) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const y = 65 - rise;
      particles.push(`<text x="${x}" y="${y}" font-family="monospace" font-size="12" fill="${getColor()}" opacity="${opacity * 0.7}"
  ...
  reading(t) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (phase < 4.5) {
        ...
        bits.push(`<text x="${x}" y="${y}" font-family="serif" font-size="11" fill="${getColor()}"
  ...
  running(t) {
    ...
    return `<g>
      <path d="M${95 + Math.sin(t) * 3} ${20 + drip}
        ...
        fill="none" stroke="${getColor()}" stroke-width="1.5" stroke-linecap="round"
      ...
      <path d="M${105 + Math.cos(t * 1.3) * 2} ${24 + drip * 0.8}
        ...
        fill="none" stroke="${getColor()}" stroke-width="1.2" stroke-linecap="round"
  ...
  celebrate(t) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      result += `<g transform="translate(${x}, ${y}) rotate(${t * 100 + i * 45})" opacity="${opacity}">
        <line x1="${-size}" y1="0" x2="${size}" y2="0" stroke="${getColor()}" stroke-width="1.2"/>
        <line x1="0" y1="${-size}" x2="0" y2="${size}" stroke="${getColor()}" stroke-width="1.2"/>
        <line x1="${-size * 0.7}" y1="${-size * 0.7}" x2="${size * 0.7}" y2="${size * 0.7}" stroke="${getColor()}" stroke-width="0.9"/>
        <line x1="${size * 0.7}" y1="${-size * 0.7}" x2="${-size * 0.7}" y2="${size * 0.7}" stroke="${getColor()}" stroke-width="0.9"/>
      </g>`;
    }
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (i % 2 === 0) {
        ...
        result += `<rect x="${x}" y="${y}" width="4" height="2.5" rx="0.5"
          fill="none" stroke="${getColor()}" stroke-width="0.8"
  ...
  error(t) {
    ...
    const bang = bangOp > 0 ? `<g opacity="${bangOp}" transform="translate(75, 35) scale(${bangScale})">
      <path d="M-15 -5 L-10 -15 L-5 -7 L3 -20 L7 -8 L15 -13 L10 -2 L20 0 L8 5 L13 13 L3 7 L-3 15 L-7 5 L-17 10 L-12 1 L-20 -2Z"
        fill="none" stroke="${getColor()}" stroke-width="1.5"/>
    </g>` : '';
  ...
  user_typing(t) {
    ...
    for (let i = 0; i < pileCount; i++) {
      ...
      bits.push(`<text x="${x + wobX}" y="${y}" font-family="monospace" font-size="${size}" fill="${getColor()}"
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const size = 10 + (i % 3) * 2;
      bits.push(`<text x="${x}" y="${y}" font-family="monospace" font-size="${size}" fill="${getColor()}"
